...
class UpdateHikeFromOpenrunnerJob < ApplicationJob
    ...
    def perform(hike)
        ...
        begin
            Rails.logger.debug { "🔗 Mise à jour de la randonnée #{hike.trail_name}" }
            ...
            begin
                ...
                distance = distance_element.find('.or-parcours-info-text').text.tr(',', '.').to_f
            ...
            if updates.any?
                ...
                hike.update(updates)
                Rails.logger.debug { "✅ Mise à jour réussie avec: #{updates}" }
            else
                hike.update(updating: false, last_update_attempt: Time.current)
                Rails.logger.debug '⚠️ Aucune donnée trouvée'
            end
        rescue StandardError => e
            Rails.logger.debug { "❌ Erreur lors de la mise à jour: #{e.message}" }
            hike.update(updating: false, last_update_attempt: Time.current)
            raise e
        ensure
            browser.quit
